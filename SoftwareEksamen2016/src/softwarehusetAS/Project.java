...
public class Project {
	...
	public List<Activity> createReport() {
		...
		for (int i = 0; i < projectActivities.size(); i++) {
			System.out
					.println(projectActivities.get(i).getDescription() + " " + projectActivities.get(i).getFinished());
	...
	public String viewActivity(String activityID) {
		for (int i = 0; i < projectActivities.size(); i++) {
			if (projectActivities.get(i).getID().equals(activityID)) {
				return "" + activityID + ": Start: " + projectActivities.get(i).getStartDate() + " End: "
						+ projectActivities.get(i).getEndDate() + " Description: "
						+ projectActivities.get(i).getDescription();
			}
		}
